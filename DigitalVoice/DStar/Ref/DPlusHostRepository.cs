...
/// <summary>
/// Provides access to DPlus reflector host addresses.
/// </summary>
public static class DPlusHostRepository
{
    const string fileName = "DPlus_Hosts.txt";
    ...
    /// </summary>
    /// <param name="filePath">The path to the DPlus_Hosts.txt file.</param>
    ...
    /// <summary>
    /// Loads DPlus host data from an embedded resource into the internal dictionary <c>_hosts</c>.
    /// </summary>
    /// <remarks>
    /// The embedded resource is expected to be a text file (e.g., <c>DPlus_Hosts.txt</c>) located
    /// in the same namespace as the <c>DPlusHostsRepository</c> class and marked as <c>EmbeddedResource</c>
    ...
    public static void LoadDataFromEmbeddedResource()
    {
        ...
        var assembly = typeof(DPlusHostRepository).Assembly;

        string? ns = typeof(DPlusHostRepository).Namespace;

        if (string.IsNullOrEmpty(ns))
            throw new InvalidOperationException("Namespace of DPlusHostRepository is null or empty.");
    ...
    /// </summary>
    /// <param name="designator">The reflector designator (e.g. "REF001").</param>
